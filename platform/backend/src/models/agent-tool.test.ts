...
describe("AgentToolModel.findAll", () => {
  ...
  describe("bulkCreateForAgentsAndTools", () => {
    ...
    test("updates null credential sources on existing assignments when options provided", async ({
      makeAgent,
      makeTool,
      makeAgentTool,
      makeMcpServer,
    }) => {
      const agent = await makeAgent({ name: "Agent with proxy tools" });
      const tool1 = await makeTool({ name: "cred-update-tool-1" });
      const tool2 = await makeTool({ name: "cred-update-tool-2" });
      const mcpServer = await makeMcpServer({ name: "New MCP Server" });

      // Create existing assignments with null credentials (simulating proxy discovery)
      await makeAgentTool(agent.id, tool1.id);
      await makeAgentTool(agent.id, tool2.id);

      // Verify credentials are null initially
      const beforeAssignments = await AgentToolModel.findAll({
        filters: { agentId: agent.id, excludeArchestraTools: true },
        skipPagination: true,
      });
      const beforeRelevant = beforeAssignments.data.filter(
        (at) => at.tool.id === tool1.id || at.tool.id === tool2.id,
      );
      for (const at of beforeRelevant) {
        expect(at.credentialSourceMcpServerId).toBeNull();
        expect(at.executionSourceMcpServerId).toBeNull();
      }

      // Bulk create with options — should update existing null-credential assignments
      await AgentToolModel.bulkCreateForAgentsAndTools(
        [agent.id],
        [tool1.id, tool2.id],
        {
      ...
      const afterAssignments = await AgentToolModel.findAll({
        filters: { agentId: agent.id, excludeArchestraTools: true },
        skipPagination: true,
      });
      const afterRelevant = afterAssignments.data.filter(
        (at) => at.tool.id === tool1.id || at.tool.id === tool2.id,
      );
      for (const at of afterRelevant) {
        expect(at.credentialSourceMcpServerId).toBe(mcpServer.id);
        expect(at.executionSourceMcpServerId).toBe(mcpServer.id);
      }
    });

    test("does not overwrite non-null credential sources on existing assignments", async ({
      makeAgent,
      makeTool,
      makeAgentTool,
      makeMcpServer,
    }) => {
      const agent = await makeAgent({ name: "Agent with existing creds" });
      const tool = await makeTool({ name: "no-overwrite-tool" });
      const originalServer = await makeMcpServer({
        name: "Original MCP Server",
      });
      const newServer = await makeMcpServer({ name: "New MCP Server" });

      // Create existing assignment with non-null credentials
      await makeAgentTool(agent.id, tool.id, {
        credentialSourceMcpServerId: originalServer.id,
        executionSourceMcpServerId: originalServer.id,
      });

      // Bulk create with different options — should NOT overwrite existing credentials
      await AgentToolModel.bulkCreateForAgentsAndTools([agent.id], [tool.id], {
        credentialSourceMcpServerId: newServer.id,
        executionSourceMcpServerId: newServer.id,
      });
      ...
      const relevantAssignment = afterAssignments.data.find(
        (at) => at.tool.id === tool.id,
      );
      expect(relevantAssignment?.credentialSourceMcpServerId).toBe(
        originalServer.id,
      );
      expect(relevantAssignment?.executionSourceMcpServerId).toBe(
        originalServer.id,
      );
    });
  });
